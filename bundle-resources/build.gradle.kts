...
mavenPublishing {
    ...
    coordinates(
        ...
        artifactId = "bundle-resources",
        version = publishVersion ?: "0.0.0-LOCAL"
    )

    pom {
        name.set("Bundle Resources")
        description.set("Resource resolution API for bundle applications with platform-specific priority")
        ...
        if (publishVersion == null) {
            throw GradleException(
                "VERSION_NAME must be set when publishing to Maven Central. " +
                        "Use: ./gradlew :bundle-resources:publishAndReleaseToMavenCentral -PVERSION_NAME=x.y.z"
            )
        }
    }
}
